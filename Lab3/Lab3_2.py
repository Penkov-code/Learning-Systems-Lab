...
if __name__ == "__main__":
    recenica = input()
    train_setFirst = [x for (x, y) in train_data]
    train_setLast = [y for (x, y) in train_data]
    cl = NaiveBayes(get_words)

    for doc in train_data:
        cl.train(doc[0], doc[1])

    words = sorted(cl.get_features(recenica))

    for word in words:
        for klasa in ["science", "sport"]:
            print(word, klasa, round(cl.get_feature_per_category_probability(word, klasa),5), round(cl.weighted_probability(word, klasa, cl.get_feature_per_category_probability),5))

    predicted_class = cl.classify_document(recenica)
    print(predicted_class, round(log2(cl.get_category_probability_for_document(recenica, predicted_class)), 5))
    #b = cl.get_feature_per_category_probability(x, "science")
    #print(b)
