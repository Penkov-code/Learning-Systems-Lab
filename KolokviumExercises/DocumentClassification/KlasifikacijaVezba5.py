...
def train_classifier(classifier: DocumentClassifier, data: list):
    for tuple in data:
        classifier.train(tuple[0],tuple[1])
...
if __name__ == '__main__':
    ...
    nb2 = NaiveBayes(get_words_to_include)

    for doc in data:
        nb1.train(doc[0], doc[1])
        nb2.train(doc[0], doc[1])

    print(f'Klasa predvidena so site zborovi: {nb1.classify_document(comment, "unknown")}')
    print(f'Klasa predvidena so samo kluchni zborovi: {nb2.classify_document(comment, "unknown")}')