...
if __name__ == '__main__':
    ...
    train_set1 = train_data[:int(len(train_data)*0.4)]
    train_set2 = train_data[int(len(train_data)*0.4):int(len(train_data)*0.8)]
    test_set = train_data[int(len(train_data)*0.8):]

    nb1 = NaiveBayes(get_words)
    nb2 = NaiveBayes(get_words)

    for doc in train_set1:
        nb1.train(doc[0], doc[1])
    for doc in train_set2:
        nb2.train(doc[0], doc[1])

    counter = 0

    for test in test_set:
        cl1 = nb1.classify_document(test[0])
        cl2 = nb2.classify_document(test[0])
        if cl1 == test[-1] and cl2 == test[-1]:
            counter += 1

    print(counter / len(test_set))
    if nb1.classify_document(sample) == nb2.classify_document(sample):
        print(nb1.classify_document(sample))
    else:
        print("unknown")