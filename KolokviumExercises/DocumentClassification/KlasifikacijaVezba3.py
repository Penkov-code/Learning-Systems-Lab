...
if __name__ == '__main__':
    sample = input()
    train_set = train_data[:int(len(train_data)*0.7)]
    test_set = train_data[int(len(train_data)*0.7):]

    nb = NaiveBayes(get_five_words)

    for doc in train_set:
        nb.train(doc[0], doc[1])

    counter = 0

    for test in test_set:
        cl1 = nb.classify_document(test[0])
        if cl1 == test[-1]:
            counter += 1

    print(counter/len(test_set))
    print(nb.classify_document(sample))