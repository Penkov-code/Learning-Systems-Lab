...
if __name__ == "__main__":
    ...
    data_set1 = train_data[:int(len(train_data)*0.5)]
    data_set2 = train_data[int(len(train_data) * 0.5):]

    for i in test_indices:
        if i < len(data_set1):
            test_set.append(data_set1[i])
    ...
    counter1 = 0
    counter2 = 0
    tree1 = build_tree(data_set1)
    tree2 = build_tree(data_set2)

    for test in test_set:
        cl1 = list(classify(test, tree1))[0]
        cl2 = list(classify(test, tree2))[0]
    ...
    acc2 = counter2 / len(test_set)
    print("Tochnost na drvoto 1: " + str(acc1))
    print("Tochnost na drvoto 2: " + str(acc2))
    if acc1 > acc2:
